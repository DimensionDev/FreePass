...
describe("Firefly Pass test", () => {
  ...
  it("#Case described in doc", async () => {
    ...
    await expect(pass.connect(user4).freeMint(proofUser4)).to.be.revertedWith("Pass: Already minted");
    await pass.connect(signers[5]).freeMint([constants.HashZero]);
    expect(await pass.balanceOf(await signers[5].getAddress())).to.be.eq(1);
    await expect(pass.connect(signers[5]).freeMint([constants.HashZero])).to.be.revertedWith("Pass: Already minted");
    //#endregion
    ...
    await expect(pass.connect(signers[6]).freeMint([constants.HashZero])).to.be.revertedWith(
    ...
    await pass.connect(signers[6]).freeMint([constants.HashZero]);
    ...
    await expect(pass.connect(signers[7]).freeMint([constants.HashZero])).to.be.revertedWith("Pass: Not in whitelist");
  ...
  describe("Test for each function", () => {
    ...
    it("Should airdropBatch works fine", async () => {
      ...
      expect(result.startTokenId.toNumber()).to.be.eq(0);
      expect(result.endTokenId.toNumber()).to.be.eq(99);
  ...
  async function checkMinted(indexes: number[]) {
    for (const index of indexes) {
      await expect(pass.connect(signers[index]).freeMint([constants.HashZero])).to.be.revertedWith(
        "Pass: Already minted",
      );
    }
  }
